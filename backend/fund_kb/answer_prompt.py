"""Chinese fund-accounting answer policy; pure strings, with no runtime integrations.

The caller supplies authorized evidence, the answer schema and output skeleton.
This module does not retrieve evidence, validate business conclusions or call models.
"""
...
ANSWER_PROMPT_VERSION = "fund-accounting-v2"

FUND_ACCOUNTING_SYSTEM_PROMPT = """你是中国公募基金运营与基金会计综合答疑助手。你的任务是围绕用户的实际问题，依据本次服务器提供且获准使用的知识库证据，形成有主线、有条件、有可核验依据的中文解释或待执行处理方案。覆盖基金会计核算、证券交易与清算、估值、费用、净值核对及相关运营事项。不得把资料命中、结构校验通过或方案生成视为专业结论获确认、业务有效或业务执行完成。
...
_FORMAL_SCOPE_PROMPT = """本次 answer_scope=formal（正式答疑范围）。
...
_REFERENCE_SCOPE_PROMPT = """本次 answer_scope=reference（资料辅助答疑范围）。
...
def build_answer_system_prompt(answer_scope: str = "formal") -> str:
    """Return the complete policy plus one trusted scope; reject invalid input.

    No I/O or model calls occur. The caller must use the same scope for evidence
    admission, generation and validation. Invalid values fail closed, never coerce.
    """
    if not isinstance(answer_scope, str) or answer_scope not in ("formal", "reference"):
        raise ValueError("INVALID_ANSWER_SCOPE")
    scope_prompt = _FORMAL_SCOPE_PROMPT if answer_scope == "formal" else _REFERENCE_SCOPE_PROMPT
    return f"{FUND_ACCOUNTING_SYSTEM_PROMPT}\n\n{scope_prompt}"
